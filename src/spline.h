//
// Created by mfc on 27.10.2017.
//

#ifndef PATH_PLANNING_SPLINE_H
#define PATH_PLANNING_SPLINE_H
/*
 * spline.h
 *
...
// LR-Decomposition of a band matrix
    void band_matrix::lu_decompose() {
      ...
      for (int k = 0; k < this->dim(); k++) {
        i_max = std::min(this->dim() - 1, k +
                                          this->num_lower());  // num_lower not a mistake!
        for (int i = k + 1; i <= i_max; i++) {
          assert(this->operator()(k, k) != 0.0);
          x = -this->operator()(i, k) / this->operator()(k, k);
          this->operator()(i,
                           k) = -x;                         // assembly part of L
...
// -----------------------
    ...
    double spline::operator()(double x) const {
      ...
      if (x < m_x[0]) {
        ...
        // interpolation
        interpol =
          ((m_a[idx] * h + m_b[idx]) * h + m_c[idx]) * h + m_y[idx];
      }
      return interpol;
    }
